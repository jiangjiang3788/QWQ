(function () {
    ...
    function ensureVectorMemoryState(chat) {
        ...
        if (!state.injectionPolicy || typeof state.injectionPolicy !== 'object') state.injectionPolicy = { budget: 2600, priority: 40 };
        if (!Number.isFinite(Number(state.injectionPolicy.budget))) state.injectionPolicy.budget = 2600;
        if (!Number.isFinite(Number(state.injectionPolicy.priority))) state.injectionPolicy.priority = 40;
    ...
    function applyVectorInjectionBudget(chat, block) {
        ensureVectorMemoryState(chat);
        const budget = Math.max(0, Number(chat.vectorMemory.injectionPolicy?.budget) || 2600);
        const text = String(block || '');
        if (!budget || text.length <= budget) return text;
        return text.slice(0, budget) + `
…[向量记忆已按预算裁剪 ${text.length - budget} 字符]`;
    }
    ...
    function getVectorMemoryContextBlock(chat, options = {}) {
        ...
        const block = applyVectorInjectionBudget(chat, buildContextBlock(chat, entries, queryText));
    ...
    async function prepareVectorMemoryContext(chat, options = {}) {
        ...
        const block = applyVectorInjectionBudget(chat, buildContextBlock(chat, selectedEntries, queryText));
        chat.vectorMemory.lastContextBlock = block;
    ...
    function renderVectorMemoryScreen() {
        ...
        const injectionBudget = document.getElementById('vector-memory-injection-budget');
        const injectionPriority = document.getElementById('vector-memory-injection-priority');
        const injectionPreview = document.getElementById('vector-memory-injection-preview');
        const loadInjectionPolicy = () => {
            const chat = getCurrentVectorChat();
            if (!chat) return;
            ensureVectorMemoryState(chat);
            if (injectionBudget) injectionBudget.value = String(chat.vectorMemory.injectionPolicy.budget || 2600);
            if (injectionPriority) injectionPriority.value = String(chat.vectorMemory.injectionPolicy.priority || 40);
        };
        document.getElementById('vector-memory-save-injection-btn')?.addEventListener('click', async () => {
            const chat = getCurrentVectorChat();
            if (!chat) return showToast('请先选择角色');
            ensureVectorMemoryState(chat);
            chat.vectorMemory.injectionPolicy = {
                budget: Math.max(0, Number(injectionBudget?.value) || 2600),
                priority: Math.min(99, Math.max(1, Number(injectionPriority?.value) || 40))
            };
            clearVectorContextCache(chat);
            await saveCharacter(chat.id);
            showToast('向量注入策略已保存');
        });
        document.getElementById('vector-memory-preview-injection-btn')?.addEventListener('click', () => {
            const chat = getCurrentVectorChat();
            if (!chat || !injectionPreview) return showToast('请先选择角色');
            ensureVectorMemoryState(chat);
            const ids = chat.vectorMemory.lastRetrievedEntryIds || [];
            const entries = ids.length
                ? chat.vectorMemory.entries.filter(item => ids.includes(item.id))
                : chat.vectorMemory.entries.slice(0, Math.max(1, Number(chat.vectorMemory.topK) || 5));
            const raw = chat.vectorMemory.lastContextBlock || buildContextBlock(chat, entries, chat.vectorMemory.lastQueryText || '预览');
            const text = applyVectorInjectionBudget(chat, raw);
            injectionPreview.textContent = [
                `优先级：${chat.vectorMemory.injectionPolicy.priority || 40}`,
                `预算：${chat.vectorMemory.injectionPolicy.budget || 2600} 字符`,
                `条目：${entries.length} 条`,
                '',
                text || '[当前没有可注入的向量记忆]'
            ].join('\n');
            injectionPreview.hidden = false;
        });
        loadInjectionPolicy();
