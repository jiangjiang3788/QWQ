...
function renderTutorialContent() {
    ...
    // V5: update log section intentionally omitted.
    ...
    if (hapticSwitch) {
        ...
        hapticSwitch.addEventListener('change', async (e) => {
            const nextValue = e.target.checked;
            db.hapticEnabled = nextValue;
            try {
                if (typeof saveGlobalSettings === 'function') await saveGlobalSettings();
                else if (typeof saveData === 'function') await saveData();
            } catch (error) {
                console.error('[Settings] 保存触感反馈设置失败:', error);
                db.hapticEnabled = !nextValue;
                e.target.checked = !nextValue;
                if (typeof showToast === 'function') showToast('触感反馈设置保存失败');
                return;
            }
...
// 单用户精简版：备份中排除已退休功能数据，但完整保留结构化档案、向量记忆、世界书、小剧场等核心数据。
const RETIRED_BACKUP_KEYS = new Set([
    'groups',
    'peekApiSettings', 'peekApiPresets',
    'globalCallWallpaper',
    'forumPosts', 'forumBindings', 'forumUserProfile', 'forumSettings', 'forumApiSettings',
    'forumMessages', 'forumStrangerProfiles', 'forumFriendRequests', 'forumPendingRequestFromUser',
    'forumAltAccounts', 'forumActiveAccountId',
    'pomodoroTasks', 'pomodoroSettings',
    'insWidgetSettings', 'homeWidgetSettings', 'homeWidgetPresets', 'widgetWallpaperPresets', 'nightModeSettings', 'homeScreenMode', 'galleryPresets',
    'hasSeenVideoCallDisclaimer', 'hasSeenVideoCallAvatarHint',
    'piggyBank', 'nodeTemplates', 'nodeSummaryText'
]);
...
function sanitizeCharacterForBackup(character) {
    const clean = cloneForBackup(character) || {};
    [
        'peekData', 'callHistory', 'familyCardEnabled', 'videoCallEnabled',
        'shopInteractionEnabled', 'nodes', 'activeNodeId'
    ].forEach(key => delete clean[key]);
    return clean;
}

function sanitizeBackupPayload(source) {
    const clean = cloneForBackup(source) || {};
    RETIRED_BACKUP_KEYS.forEach(key => delete clean[key]);
    clean.characters = Array.isArray(clean.characters)
        ? clean.characters.map(sanitizeCharacterForBackup)
        : [];
    delete clean.groups;
    return clean;
}

function sanitizeImportedPayload(source) {
    const clean = sanitizeBackupPayload(source);
    // 兼容旧备份：只忽略退休字段，不影响角色私聊、档案、向量、世界书、小剧场等核心数据。
    clean._exportVersion = source && source._exportVersion ? source._exportVersion : 'legacy';
    if (source && source._exportTimestamp) clean._exportTimestamp = source._exportTimestamp;
    if (source && source._exportTables) clean._exportTables = source._exportTables.filter(k => !RETIRED_BACKUP_KEYS.has(k));
    return clean;
}

// 创建完整的备份数据（确保主题预设、屏幕预设等 globalSettingKeys 全部导出）
async function createFullBackupData() {
    const backupData = sanitizeBackupPayload(db);
    const keys = window.globalSettingKeysForBackup || [];
    keys.forEach(k => {
        if (RETIRED_BACKUP_KEYS.has(k)) return;
        if (db[k] !== undefined && backupData[k] === undefined) {
            backupData[k] = cloneForBackup(db[k]);
        }
    });
    backupData._exportVersion = '3.1_single_user';
...
async function createPartialBackupData(selectedKeys) {
    ...
    for (const key of selectedKeys) {
        if (key === 'globalSettings') {
            ...
            keys.forEach(k => { if (!RETIRED_BACKUP_KEYS.has(k)) result.globalSettings[k] = cloneForBackup(db[k]); });
            ...
            result.characters = (db.characters || []).map(sanitizeCharacterForBackup);
        } else if (!RETIRED_BACKUP_KEYS.has(key) && db[key] !== undefined) {
            result[key] = cloneForBackup(db[key]);
        }
    }
    return result;
}
...
async function importPartialBackupData(data) {
    ...
    try {
        ...
        for (const key of tables) {
            if (RETIRED_BACKUP_KEYS.has(key)) continue;
            if (key === 'globalSettings' && data.globalSettings) {
                Object.keys(data.globalSettings).forEach(k => { if (!RETIRED_BACKUP_KEYS.has(k)) db[k] = data.globalSettings[k]; });
            } else if (key === 'theaterData' && data.theaterData) {
                Object.keys(data.theaterData).forEach(k => { db[k] = data.theaterData[k]; });
            } else if (key === 'characters' && Array.isArray(data.characters)) {
                db.characters = data.characters.map(sanitizeCharacterForBackup);
            } else if (!RETIRED_BACKUP_KEYS.has(key) && data[key] !== undefined) {
                db[key] = cloneForBackup(data[key]);
            }
        }
        showToast('正在写入...');
        await saveData(db);
...
async function importBackupData(data) {
    ...
    try {
        ...
        if (!['3.0', '3.1_single_user'].includes(data._exportVersion)) {
        ...
        convertedData = sanitizeImportedPayload(convertedData);

        // 清除当前内存里可能残留的退休字段，避免清空数据库后又被 saveData 写回。
        RETIRED_BACKUP_KEYS.forEach(key => { delete db[key]; });
        db.groups = [];

        // 从备份恢复所有保留键；退休字段会被安全忽略。
