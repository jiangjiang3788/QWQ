...
assert(chatAi.includes('<user_favorites count="${userLines.length}">'));
assert(chatAi.includes('<character_favorites count="${characterLines.length}">'));
assert(chatAi.includes("item?.favoriteBy !== 'character'"));
assert(chatAi.includes("item?.favoriteBy === 'character'"));
assert(chatAi.includes("title: '收藏盘点'"));
assert(chatAi.includes('favoriteCounts'));
...
console.log('V5.6.5 request panel test passed.');
