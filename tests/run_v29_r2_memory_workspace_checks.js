...
assert(css.includes('.memory-workbench-card-grid'), 'workspace card layout is missing');
...
assert(workspace && workspace.VERSION === '2.9-R2', 'workspace module did not register');
...
assert(workspace.getWorkspaceForView('feedback') === 'inbox', 'feedback should be under inbox');
