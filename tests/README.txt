...
V2.14-R9 生命周期维护闭环、来源变化链与 Proment 桌面修复：
- node tests/run_v214_r9_lifecycle_provenance_and_proment_checks.js
- 验证 Proment 在桌面应用网格中仅显示一次并打开现有 Proment 页面。
- 验证生命周期健康报告和维护预演为只读，执行后只改变状态并保留来源变化链。
- 验证创建、字段更新、Upsert、确认、归档、过期和合并记录统一来源与变化事件。
- 验证迁移快照重映射变化链中的行引用并移除源角色消息、事务和操作引用。


V2.15-R0A 无损界面收敛：
- node tests/run_v215_r0a_lossless_ui_convergence_checks.js
- 验证桌面和 Dock 从单一应用注册表 placement 生成，Proment 不再依赖平行 ID 数组。
- 验证普通表只显示“自动生效 / 待确认 / 仅人工”，长期晋升仅在长期候选职责中显示。
- 验证审核和候选的底层模式保持不变，角色覆盖保存时正确映射。
- 验证管理页集中为记忆诊断中心，任务队列用户文案收敛为维护作业。

V2.15-R0B 字段语义与硬编码消除：
- node tests/run_v215_r0b_semantic_identity_hardcode_checks.js
- 验证默认模板 8 张表、94 个字段具有显式 semanticRole 与 identityRole。
- 验证字段显示名称变化后，记录身份、Sidecar 写入和长期晋升仍按稳定语义运行。
- 验证 Schema 3.1→3.2 迁移只修改副本中的模板元数据，不修改正式记忆。
- 验证固定短期表 ID、表格中文职责正则和技术字段中文正则不再参与运行时业务路由。
- 验证生命周期与检索关键默认值从统一 memoryDefaults 读取。
