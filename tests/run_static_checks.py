...
# 结构化记忆 V2 回归检查。
for rel in ['js/modules/message_content.js','js/modules/memory_table_schedule.js','js/features/memory/kernel.js','js/features/memory/api_adapter.js','js/features/memory/domain.js','js/features/memory/schema_model.js','js/features/memory/schema_editor.js','js/features/memory/facade.js','js/modules/memory_table_policy.js','js/modules/memory_table_lifecycle.js','js/modules/memory_table_effects.js','js/modules/memory_table_feedback.js','js/modules/memory_table_review.js','js/features/memory/retrieval_maintenance.js','js/modules/memory_table_retrieval.js','js/features/memory/retrieval_audit.js','js/modules/memory_table_sidecar.js','js/modules/memory_table_tasks.js','css/modules/memory_table_v2.css','memory_templates/当前默认记忆模板_V2.8.json']:
    if not (root/rel).exists(): errors.append('missing memory v2 asset: '+rel)
for rid in ['memory-table-normal-mode-btn','memory-table-json-mode-btn','memory-table-open-schema-editor-btn','memory-schema-editor-modal','memory-schema-editor-body','memory-table-trigger-mode','memory-table-round-interval','memory-table-auto-update-toggle','memory-table-auto-update-interval','memory-table-max-source-messages','memory-table-update-latest-btn','memory-table-retry-btn','memory-table-retrieval-mode','memory-table-semantic-weight','memory-table-embedding-candidate-limit','memory-review-tab-count','memory-range-preview-modal','memory-live-state-bar','memory-sidecar-enabled-toggle','memory-sidecar-candidate-toggle','memory-sidecar-statusbar-toggle','memory-sidecar-tab-count','memory-table-tag-weight','memory-table-scene-routing-toggle','memory-table-side-effect-guard-toggle','memory-task-tab-count','memory-usage-audit-tab-count']:
...
retrieval_maintenance_js=(root/'js/features/memory/retrieval_maintenance.js').read_text(encoding='utf-8') if (root/'js/features/memory/retrieval_maintenance.js').exists() else ''
...
if 'buildIndexPlan' not in retrieval_maintenance_js or 'recordUsage' not in retrieval_maintenance_js or 'clearIndex' not in retrieval_maintenance_js: errors.append('memory v2.14-R4 retrieval maintenance missing')
if 'markRetrieved(' in retrieval_js or 'MemoryEffects.markInjected(' in memory_table_js: errors.append('memory retrieval still mutates formal row usage during prompt assembly')
...
for token in ["Kernel.register('policy'", "Kernel.register('lifecycle'", "Kernel.register('effects'", "Kernel.register('feedback'", "Kernel.register('review'", "Kernel.register('retrieval'", "Kernel.register('retrievalAudit'", "Kernel.register('retrievalMaintenance'", "Kernel.register('sidecar'", "Kernel.register('tasks'", "Kernel.register('quality'", "Kernel.register('controller'"]:
    if token not in ''.join([policy_js,lifecycle_js,effects_js,feedback_js,review_js,retrieval_maintenance_js,retrieval_js,(root/'js/features/memory/retrieval_audit.js').read_text(encoding='utf-8'),sidecar_js,tasks_js,(root/'js/modules/memory_table_quality.js').read_text(encoding='utf-8'),memory_table_js]): errors.append('memory kernel registration missing: '+token)
